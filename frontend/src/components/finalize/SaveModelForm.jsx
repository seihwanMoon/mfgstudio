  ...
  return (
    <div style={{ border: "1px solid #1A3352", borderRadius: 14, background: "#0D1926", padding: 16 }}>
      ...
      <code style={{ color: "#38BDF8", fontSize: 12, display: "block", marginBottom: 12 }}>
        {modelPath || "아직 finalize 되지 않았습니다."}
      </code>

      <div style={{ color: "#E2EEFF", fontWeight: 700, marginBottom: 8 }}>리포트</div>
      {reportUrl ? (
        <a
          href={reportUrl}
          target="_blank"
        ...
        >
          {reportGenerated ? "생성된 PDF 리포트 열기" : "최신 PDF 리포트 열기"}
        </a>
      ) : (
        <div style={{ color: "#8BA8C8", fontSize: 12 }}>아직 리포트를 사용할 수 없습니다.</div>
      )}

      {reportError ? (
        <div style={{ color: "#FCA5A5", fontSize: 12, marginTop: 10 }}>리포트 경고: {reportError}</div>
      ) : null}
    </div>
  )
}
